...
import { buildDateWiseRoleCounts, getRoleCountsForDate } from "../../utils/utils1";
...
export const ResourceAvailability = ({
  ...
  // Per-date TL/EX counts (requirement: counted per date via dateWiseAssignments,
  // not by counting allocation rows — one row can span many dates).
  const dateWiseRoleCounts = useMemo(
    () => buildDateWiseRoleCounts(dateWiseAssignments || {}),
    [dateWiseAssignments]
  );

  // Headline pills summarize the busiest day in the currently displayed window,
  // so the "Short N" warning still fires if any single date in view is under target.
  const { peakTL, peakEX } = useMemo(() => {
    let tl = 0, ex = 0;
    dayWindow.forEach((d) => {
      const counts = getRoleCountsForDate(dateWiseRoleCounts, formatToApiDate(d));
      tl = Math.max(tl, counts.TL);
      ex = Math.max(ex, counts.EX);
    });
    return { peakTL: tl, peakEX: ex };
  }, [dayWindow, dateWiseRoleCounts]);
  ...
  const columns = useMemo(() => {
    ...
    dayWindow.forEach((d) => {
      ...
      const { TL, EX } = getRoleCountsForDate(dateWiseRoleCounts, formatToApiDate(d));
      cols.push(`${num} ${dow} (T${TL}/E${EX})`);
    });
    ...
    >
      <InfoStrip>
        ...
        <InfoPill>
          ...
          {peakTL} / {plannedTL}
          {peakTL < plannedTL && (
            <ShortPill>Short {plannedTL - peakTL} TL</ShortPill>
          )}
        </InfoPill>
        <InfoPill>
          <FaUser size={10} />
          ...
          {peakEX} / {plannedEX}
          {peakEX < plannedEX && (
            <ShortPill>Short {plannedEX - peakEX} EX</ShortPill>
          )}
        </InfoPill>
      </InfoStrip>
