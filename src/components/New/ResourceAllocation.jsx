...
const ResourceAllocation = () => {
  ...
  // ---- Week navigation (Resource Availability calendar only) ----
  // start/end already cover the full month (see getMonthRange() above), and
  // busy/current allocations for that whole range are already fetched in
  // loadAllData(). Navigating weeks is pure UI — no extra API calls.
  const [weekOffset, setWeekOffset] = useState(0);

  const monthDates = useMemo(() => generateDateRange(start, end), [start, end]);
  ...
  const currentWeek = availabilityWeeks[weekOffset] || [];

  const handlePrevWeek = () => setWeekOffset((w) => Math.max(0, w - 1));
  const handleNextWeek = () => setWeekOffset((w) => Math.min(availabilityWeeks.length - 1, w + 1));
  ...
  const isLocked = (row) => isPlanRowLocked(row, activityStart);
        ...
        {showResourceAvailability && (
          <>
            <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: "0.75rem", marginBottom: "0.5rem" }}>
              <Button size="sm" variant="outline" onClick={handlePrevWeek} disabled={weekOffset === 0}>‹ Prev Week</Button>
              <span style={{ fontSize: "0.8rem", fontWeight: 500 }}>{formatWeekLabel(currentWeek[0], currentWeek[currentWeek.length - 1])}</span>
              <Button size="sm" variant="outline" onClick={handleNextWeek} disabled={weekOffset === availabilityWeeks.length - 1}>Next Week ›</Button>
            </div>
            <ResourceAvailability
              employees={employees}
              dayWindow={currentWeek}
              activityData={activityData}
              activityDates={currentWeek}
              activityStart={activityStart}
              activityEnd={activityEnd}
