...
import { formatDate, formatMonthLabel, formatToDDMMYYYY, formatWeekLabel, getMonthRange, getStatusVariant, groupByOrderItemId, matchClaimsToActivity } from '../../utils/utils';
...
import { FaCheck, FaClipboardList, FaEye, FaEyeSlash, FaHourglassEnd, FaMinusCircle, FaMoneyBillWave, FaUserCheck, FaUserPlus, FaUserTimes } from 'react-icons/fa';
...
const ActivityListScreen = () => {
	...
	const enrichedAssignedActivity = useMemo(
		() => enrichActivitiesWithClaims(assignedActivity, claimList),
		[assignedActivity, claimList, enrichActivitiesWithClaims]
	);

	const groupedData = groupByOrderItemId(enrichedAssignedActivity, resourcePlannedList);
								...
								<Td>
									<ButtonGroup>
										{(employee.activityStatus === "C" || employee.activityStatus === "AP" || employee.activityStatus === "AS") ? (
											<Button
												size='sm'
												onClick={() => navigate('/clamDetails', { state: { data: { ...employee, claims: employee?.claims || [], mode: "ADD" } } })}
											>
												<FaMoneyBillWave />
												Claim
											</Button>
