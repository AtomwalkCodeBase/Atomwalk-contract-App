...
/* ============================================================
 * SECTION: NORMALIZATION (raw API -> UI shape)
 * ============================================================ */
...
/* ============================================================
 * SECTION: NORMALIZATION - resource allocation rows
 * ============================================================ */
...
/* ============================================================
 * SECTION: LEGACY (action-flag model) — pre-dates the
 * workingAllocations/originalAllocations diff model below.
 * Not called from ResourceAllocation.jsx, CurrentAssignments.jsx,
 * or ResourceAvailability.jsx as of this refactor. Kept here
 * (not deleted) in case another screen outside this module still
 * imports them. Verify with a repo-wide search before removing.
 * ============================================================ */
...
/* --- legacy, unused by current screens, see note above --- */
...
/* ============================================================
 * SECTION: SOURCE-OF-TRUTH MODEL (workingAllocations diff engine)
 * ============================================================ */
...
export const getRowStatus = (row, originalById) => {
  ...
  const changed =
    ...
    fieldsChanged(row, original, PLAN_EDITABLE_FIELDS);
...
/**
 * Configurable editable fields (Plan). Add future fields (e.g.
 * contract_rate) here only — buildPayloads/getRowStatus below
 * already read off this list, so no business logic changes needed.
 */
export const PLAN_EDITABLE_FIELDS = ["remarks", "emp_type"];

const fieldsChanged = (row, original, fields) =>
  fields.some((f) => (row[f] ?? "") !== (original[f] ?? ""));
 ...
 * Requirement: plan is fully locked once Today >= Activity Start
 * Date; individually, any row with is_approved === true is locked
 * regardless of plan lock state. Actual rows lock on is_present
 * OR is_approved. One source of truth for all three screens.
 * ============================================================ */

/** Whole-plan lock: true once today has reached the activity start date. */
export const isPlanLocked = (activityStart) => {
  if (!activityStart) return false;
  const startComparable = DateForApiFormate(activityStart, true);
  if (!startComparable) return false;
  const todayComparable = DateForApiFormate(new Date(), true);
  return todayComparable >= startComparable;
};

/** Single plan row lock: approved rows are always read-only, independent of plan lock. */
export const isPlanRowLocked = (row, activityStart) =>
  !!row?.is_approved || isPlanLocked(activityStart);

/** Actual row lock: cannot edit once present or approved. */
export const isActualRowLocked = (row) =>
  row?.is_present === true || row?.is_approved === true;

/* ============================================================
 * SECTION: TL / EX COUNTS (per date, not per allocation row)
 * Requirement: counts must be computed PER DATE using
 * dateWiseAssignments, since one row can span many dates and a
 * simple row filter double/under counts days.
 * ============================================================ */

/** { 'DD-Mon-YYYY': { TL, EX } } built from dateWiseAssignments. */
export const buildDateWiseRoleCounts = (dateWiseAssignments = {}) => {
  const counts = {};
  Object.entries(dateWiseAssignments).forEach(([date, rows]) => {
    const tl = rows.filter((r) => r.emp_type === "T" && r.action !== "DELETE").length;
    const ex = rows.filter((r) => r.emp_type === "E" && r.action !== "DELETE").length;
    counts[date] = { TL: tl, EX: ex };
  });
  return counts;
};

/** Convenience accessor for a single date, defaults to { TL: 0, EX: 0 }. */
export const getRoleCountsForDate = (dateWiseRoleCounts, date) =>
  dateWiseRoleCounts?.[date] || { TL: 0, EX: 0 };

/* ============================================================
 * SECTION: SEARCH / FILTER HELPERS (CurrentAssignments)
 * ============================================================ */

/** Matches employee name OR emp_type ("TL"/"EX", case-insensitive) against a single search term. */
export const matchesNameOrRole = (row, search) => {
  if (!search) return true;
  const term = search.trim().toLowerCase();
  if (!term) return true;

  const nameMatch = (row.employee_name || "").toLowerCase().includes(term);

  const roleLabel = row.emp_type === "T" ? "tl" : row.emp_type === "E" ? "ex" : "";
  const roleMatch = roleLabel === term || roleLabel.startsWith(term);

  return nameMatch || roleMatch;
};

/** True if a row's date falls inside [startDate, endDate] (comparable YYYY-MM-DD or DD-Mon-YYYY, both sort-safe within same format). */
export const isDateInRange = (date, startDate, endDate) => {
  if (!date) return false;
  const d = DateForApiFormate(date, true);
  const s = startDate ? DateForApiFormate(startDate, true) : null;
  const e = endDate ? DateForApiFormate(endDate, true) : null;
